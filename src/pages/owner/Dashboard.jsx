  ...
  return (
    <div className="page">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Total Flats', value: stats.totalFlats, icon: Home, tone: 'icon-primary', accent: 'border-[var(--md-sys-color-primary)]' },
          { label: 'Rented Flats', value: stats.rentedFlats, icon: Users, tone: 'icon-secondary', accent: 'border-[var(--md-sys-color-secondary)]' },
          { label: 'Available Flats', value: stats.availableFlats, icon: Home, tone: 'icon-tertiary', accent: 'border-[var(--md-sys-color-tertiary)]' },
          { label: 'Service Requests', value: stats.serviceRequests, icon: Wrench, tone: 'icon-error', accent: 'border-[var(--md-sys-color-error)]' },
        ].map((card, i) => (
          <div key={i} className={`stat-card border-l-4 ${card.accent}`}>
            <div>
              <p className="text-sm text-slate-500">{card.label}</p>
              <p className="text-3xl font-display font-semibold text-slate-900 mt-2">
                {card.value}
              </p>
            </div>
      ...
      <div>
        ...
        {availableTenants.length === 0 ? (
          <div className="card p-10 text-center">
            ...
            <p className="text-slate-600">No available tenants right now</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {availableTenants.map(tenant => (
              <div key={tenant.unique_id} className="card-hover p-6">
                <h3 className="font-semibold text-slate-900">{tenant.username}</h3>
